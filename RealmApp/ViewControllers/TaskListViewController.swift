...
class TaskListViewController: UITableViewController {
    ...
    var taskLists = (StorageManager.shared.realm.objects(TaskList.self)).sorted(byKeyPath: "date", ascending: true)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if getCurrentTaskCount(in: indexPath.row) != 0 {
            content.secondaryText = ("\(getCurrentTaskCount(in: indexPath.row))")
        }
    ...
    private func getCurrentTaskCount(in index: Int) -> Int {
        var count = 0
        taskLists[index].tasks.forEach { task in
            if !task.isComplete {
                count += 1
            }
        }
